...
namespace STM32T
{
	...
	*/
	...
	public:
		...
		static constexpr uint8_t FONT_WIDTH = 5, FONT_HEIGHT = PIXELS_PER_LINE;
		
	private:
		static constexpr uint8_t BIT_FILTER[8] = { 0b0000'0001, 0b0000'0011, 0b0000'0111, 0b0000'1111, 0b0001'1111, 0b0011'1111, 0b0111'1111, 0b1111'1111 };
		
		static constexpr uint8_t Font[][FONT_WIDTH] =
		{
		...
		uint8_t m_cursor = 0, m_row = 0, m_page = 0;
		line_t m_line = 0;
	...
	private:
		...
		[[gnu::always_inline]] uint16_t MapIndex(line_t line, uint8_t cursor) const
		{
		...
		[[gnu::always_inline]] static constexpr line_t Line(uint8_t y)
		{
		...
		bool BusyWait(const Time::us_time_t timeout_us = 60) const
		{
		...
		* @param line - [0, 7]
		*/
		bool SetLine(line_t line, const bool force = true)
		{
			line %= MAX_LINES;
		...
		* @param cursor - [0, 63]
		*/
		bool SetCursor(uint8_t cursor, const bool force = true)
		{
			cursor %= MAX_CURSOR;
		...
		[[gnu::always_inline]] void Display(const bool display) const
		{
			Instruction(0b0011'1110 | display);
		}
		
		/**
		* @param cursor - [0, screenLen)
		* @param line - [0, 7]
		*/
		void Goto(const uint8_t cursor, const line_t line, const bool force = false)
		{
		...
		void CheckCursor(const line_t lines = 1)
		{
		...
		void Write(uint8_t byte, const bool check = true, const line_t lines = 1)
		{
			if (negate)
				byte = ~byte;
		...
		void WriteTop(uint8_t byte, const uint8_t shift, const bool check = true, const line_t lines = 1)
		{
			if (negate)
				byte = ~byte;
		...
		void WriteBottom(uint8_t byte, const uint8_t shift, const bool check = true, const line_t lines = 1)
		{
			if (negate)
				byte = ~byte;
		...
		/**
		* @note Assumes x and y are checked.
		*/
		void Line_y(const uint8_t x, const uint8_t y, const uint8_t len, const bool draw)
		{
			...
			{
				if (draw)
					Write((BIT_FILTER[rowEnd - rowBegin] << rowBegin) | m_screenMap[MapIndex(lineBegin, x)]);
				else
					Write(~(BIT_FILTER[rowEnd - rowBegin] << rowBegin) & m_screenMap[MapIndex(lineBegin, x)]);
			}
			else
			{
				...
				SetLine(lineEnd);
				SetCursor(x, true);
				Write(draw ? (BIT_FILTER[rowEnd] | m_screenMap[MapIndex(lineEnd, x)]) : (~BIT_FILTER[rowEnd] & m_screenMap[MapIndex(lineEnd, x)]), false);
	...
	public:
		...
		*/
		KS0108B(uint8_t (* const rw)(bool rw, bool rs, uint8_t data), void (*set_cs)(uint8_t), const uint8_t page_count = 2) :
			f_rw(rw), f_setCS(set_cs), m_pageCount(std::min(page_count, uint8_t(8u))), screenLen(m_pageCount * MAX_CURSOR),
			m_screenMap(new uint8_t[MAX_LINES * screenLen]) {}
		
		KS0108B(const KS0108B&) = delete;
		KS0108B(KS0108B&&) = delete;
		
		~KS0108B() { delete[] m_screenMap; }
		...
		*/
		...
		[[gnu::always_inline]] KS0108B& Clear(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, const bool fill = false)
		{
		...
		KS0108B& XL(const uint8_t x, const line_t line)
		{
			m_row = 0;
			Goto(x, line);
			return *this;
		...
		std::pair<uint8_t, line_t> GetXL() { return { m_cursor + MAX_CURSOR * m_page, m_line }; }
		...
		KS0108B& WriteByteArr(const uint8_t *const bytes, const size_t count)
		{
			if (m_row == 0)
		...
		template <size_t N>
		[[gnu::always_inline]] void WriteByteArr(const uint8_t (&bytes)[N])
		{
			WriteByteArr(bytes, N);
		}
		
		/**
		* @brief Goes to the next line and sets the cursor to 0.
		*/
		KS0108B& NextLine(const line_t lines = 1) override
		{
		...
		KS0108B& Pixel(const uint8_t x, uint8_t y, bool fill = true)
		{
		...
		KS0108B& PutChar(char ch, bool interpret_specials = true, bool auto_next_line = true) override
		{
			...
			if (interpret_specials)
			{
				switch (ch)
				{
					...
					case '\t':
					{
						// todo: add spaces up to a cursor that's a multiple of 2/4.
						PutChar(' ');
						PutChar(' ');
		...
		void PutCharBig(uint8_t ch, bool interpret_specials = true)
		{
			//todo: space between characters
		...
		void PutStrnBig(const char* str, size_t n)
		{
			while (n--)
				PutCharBig(*str++);
		}
		...
		*/
		...
		void FillRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, bool fill = true)
		{
			...
			if (line1 == line2)
			{
				for (uint8_t x = x1; x <= x2; ++x, ++index)
					Write(fill ? ((BIT_FILTER[row2 - row1] << row1) | m_screenMap[index]) : (~(BIT_FILTER[row2 - row1] << row1) & m_screenMap[index]));
			}
			else
			{
				for (uint8_t x = x1; x <= x2; ++x, ++index)
				...
				for (uint8_t x = x1; x <= x2; ++x, ++index)
					Write(fill ? (BIT_FILTER[row2] | m_screenMap[index]) : (~BIT_FILTER[row2] & m_screenMap[index]));
			}
		}
		...
		void NegateRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
		{
			...
			{
				for (uint8_t x = x1; x <= x2; ++x, ++index)
					Write(((BIT_FILTER[row2 - row1] << row1) & ~m_screenMap[index]) | (~(BIT_FILTER[row2 - row1] << row1) & m_screenMap[index]));
			}
			else
			{
				...
				for (uint8_t x = x1; x <= x2; ++x, ++index)
					Write((BIT_FILTER[row2] & ~m_screenMap[index]) | (~BIT_FILTER[row2] & m_screenMap[index]));
			}
		}
