...
class W25Q128JV
{
	...
	*/
	bool Write(const CMD cmd, const uint32_t addr, const uint8_t * const buf, const uint8_t len)
	{
...
public:
	...
	*/
	...
	{
		const uint8_t _addr[3] = { (uint8_t)(addr >> 16), (uint8_t)(addr >> 8), (uint8_t)addr };
		return Read(READ_DATA, data, len, strv((char *)_addr, sizeof(_addr)));
	}
	...
	bool WriteData(const uint32_t addr, const uint8_t * const data, const uint8_t len)
	{
