...
You need to provide the KS0108B object with a `command` function pointer to manipulate the data pins as well as the RS, RW, and EN pins according to the timing specifications of KS0108B (page 8 of [the datasheet](https://www.sparkfun.com/datasheets/LCD/ks0108b.pdf)). Here are two examples of such function:
...
void my_glcd_cmd2(uint8_t data, const bool rw, const bool rs)
{
    HAL_GPIO_WritePin(LCD_RS_GPIO_Port, LCD_RS_Pin, rs);
    HAL_GPIO_WritePin(LCD_RW_GPIO_Port, LCD_RW_Pin, rw);
    ...
    HAL_GPIO_WritePin(LCD_EN_GPIO_Port, LCD_EN_Pin, GPIO_PIN_SET);
    STM32T::DWT_Delay(5);   // microseconds
    HAL_GPIO_WritePin(LCD_EN_GPIO_Port, LCD_EN_Pin, GPIO_PIN_RESET);
    STM32T::DWT_Delay(3);
}
...
int main()
{
    ...
    KS0108B glcd2(my_glcd_cmd2, ...);
    glcd2.PutStr("Hello, World!");
}
